...
const button = tv({
  base: 'flex items-center h-full aspect-square shrink-0 justify-center items-center rounded-[calc(0.75rem_-_0.125rem)]',
  variants: {
    active: {
      true: 'bg-jaffa-950',
      false: '',
...
const TransportSelector = ({ transportType, setTransportType }: TransportSelectorProps) => {
  return (
    <div className='flex bg-jaffa-50 rounded-xl w-17 shrink-0 justify-around overflow-hidden p-0.5'>
      <div className={button({ active: transportType === '1' })} onClick={() => setTransportType('1')}>
        <Plane  className={buttonIcon({ active: transportType === '1' })}/>
        <span className='sr-only'>Авіа</span>
      </div>
      <div className={button({ active: transportType === '2' })} onClick={() => setTransportType('2')}>
